...
void GameWorld::UpdateWorld(float dt) {
	...
	for (GameObject* g : gameObjects) {
		g->Deteriorate();
		if (g->GetRenderObject()->GetTransform()->GetScale().x < 0.1) {
			g->deteriorate = false;
			deadTimer++;
		}

		if (deadTimer > 100) {
			g->GetRenderObject()->GetTransform()->SetScale(g->initialScale);
			g->SetBoundingVolume((CollisionVolume*) new AABBVolume(g->initialScale/2));
			g->deteriorate = false;
			deadTimer = 0;
		}
	}
