...
PhysicsSystem::PhysicsSystem(GameWorld& g) : gameWorld(g)	{
	...
	SetGravity(Vector3(0.0f, -98.16f, 0.0f));
}
	...
	for (auto i = first; i != last; ++i) {
		...
		for (auto j = i + 1; j != last; ++j) {
			...
			if (CollisionDetection::ObjectIntersection(*i, *j, info)) {
				if (!(info.b->IsTrigger()) && !(info.a->IsTrigger()))
					ImpulseResolveCollision(*info.a, *info.b, info.point);

				if (info.a->GetType() == ObjectType::Crumble && info.b->player == true) {
					info.a->deteriorate = true;
				}

				if (info.b->GetType() == ObjectType::Crumble && info.a->player == true) {
					info.b->deteriorate = true;
				}
...
void PhysicsSystem::ImpulseResolveCollision(GameObject& a, GameObject& b, CollisionDetection::ContactPoint& p) const {
	...
	Vector3 finertiaA = Vector3::Cross(physA->GetInertiaTensor() * Vector3::Cross(relativeA, tangent), relativeA);
	Vector3 finertiaB = Vector3::Cross(physB->GetInertiaTensor() * Vector3::Cross(relativeB, tangent), relativeB);
	float fangularEffect = Vector3::Dot(inertiaA + inertiaB, tangent);
	...
	float jt = (-cFriction * frictionForce) / (totalMass + fangularEffect);
