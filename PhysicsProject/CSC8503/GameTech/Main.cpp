...
public:
	PauseScreen(TutorialGame* game, bool GameType) { g = game; GameB = GameType; }
	PushdownResult OnUpdate(float dt, PushdownState * *newState) override {
		g->Paused(dt);
		if (Window::GetKeyboard()->KeyPressed(KeyboardKeys::P) || g->pop) {
...
public:
	GameScreen(bool Game) { GameB = Game; }
	PushdownResult OnUpdate(float dt, PushdownState ** newState) override {
		g->UpdateGame(dt);
		if (Window::GetKeyboard()->KeyPressed(KeyboardKeys::P) || g->GameEnd) {
			* newState = new PauseScreen(g, GameB);
